from agents import (
     Agent, Runner, function_tool, InputGuardrail, GuardrailFunctionOutput, InputGuardrailTripwireTriggered
)
...
budget_analysis_agent = Agent(
    ...
    instructions="""
    You Analyze travel budgets to determine if they are realistic for the destination & duration.
    ...
    Provide a clear analysis of whether the budget is realistic & why, if the budget is not realistic,
    suggest a more appropriate budget. Don't be harsh at all, lean towards it beings realistic
    unless it's really crazy. If no budget was mentioned, just assume it is realistic.
""",
